 ...
 */
internal class OldStar(starConstraints: StarConstraints, var x: Int, var y: Int, var randomizeLocation: Boolean, var opacity: Double, var color: Int, viewWidth: Int, viewHeight: Int, private val colorListener: () -> Int) {
    ...
    init {
        ...
        // the alphaInt incerment speed will be decided according to the star's size
    ...
    fun calculateFrame(viewWidth: Int, viewHeight: Int) {
        ...
        // convert to int-based alphaInt
        alpha = (opacity * 255.0).toInt()

        when {
            alpha > 255 -> {
                // reset alphaInt to full
                alpha = 255
            }
            alpha <= 0 -> {
                // reset alphaInt to 0
